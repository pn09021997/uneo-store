...
class User extends Db {
    static function getUserInfo($userId) {
        $sql = self::$connection->prepare("SELECT * FROM users where users.id = ? ");
        $sql->bind_param("s",$userId);//return an object
    ...
    static function getUserLogin($username)
    {
        //2. Viết câu SQL
    ...
    static function getAllUsers_RemoveUserId($user_id) {
        $sql = self::$connection->prepare("SELECT * FROM users WHERE id <> $user_id");
        $sql->execute();
        $items = array();
        $items = $sql->get_result()->fetch_all(MYSQLI_ASSOC);
        return $items; //return an array.
     ...
     * THÊM User:
     */
    static function insertUser($username, $password, $permission) {
        $sql = self::$connection->prepare("INSERT INTO `users`(`username`, `password`, `permission`)
        VALUES (?, ?, ?)");
        $sql->bind_param("sss", $username, $password, $permission);
        return $sql->execute();
    }

...
class CartDetail extends Db
{
    ...
    {
        $sql = self::$connection->prepare("SELECT * FROM carts_detail WHERE productid = ?");
    ...
    {
        $sql = self::$connection->prepare("UPDATE carts_detail SET quantity = ?, price = ? WHERE productid = ? AND cart_id = ?");
        $sql->bind_param('iiii', $quantity, $price, $productId, $cart_id);
        return $sql->execute();
    }

    static function removeProduct_ById($cart_id, $productId)
    {
        $sql = self::$connection->prepare("DELETE FROM carts_detail WHERE productid = ? AND cart_id = ?");
        $sql->bind_param('ii', $productId, $cart_id);
        return $sql->execute();
    }
